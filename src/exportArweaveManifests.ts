...
async function main() {
  ...
  const name = "Arweave_All_Manifests_" + today + ".csv";
  ...
  const fileDataTxs = await getAllAppL2Manifests(start, end, "N/A", minBlock);
  ...
  console.log("%s arweave manifests found", allManifests.length);
  fs.writeFile("arweave_manifests.json", data, (err) => {
    if (err) throw err;
