...
export async function main () {
    ...
    const csvWriter = createCsvWriter({
        path: name,
        header: [
            {id: 'folderId', title: 'FOLDERID'},
        ]
    });

    console.log('Getting all ArDrive Folder IDs from %s to %s', start.toLocaleString(), end.toLocaleString());
    const allFolders = await getAllFolders(start, end)

    console.log ("Total Folder IDs found: %s", allFolders.length)
    console.log ("Total Unique Folder IDs: %s", countDistinct(allFolders, allFolders.length))
    csvWriter.writeRecords(allFolders)
    .then(() => {
