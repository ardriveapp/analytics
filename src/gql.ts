...
export async function getAllAppTransactions_ASC(start: Date, end: Date, lastBlock: number) {
    ...
    if (lastBlock === 1) {
        ...
        minBlock = lastBlock;
        console.log ("Continuing querying for All App Transactions from block %s", lastBlock)
    }
...
// Uses an estimate of block time to try to reduce query size and returns oldest results first
export async function getAllAppDriveTransactions_ASC(start: Date, end: Date, lastBlock: number) {
    let firstPage : number = 100; // Max size of query for GQL
    let cursor : string = "";
    let timeStamp = new Date(end);
    let hasNextPage = true;
    ...
    if (lastBlock === 1) {
        ...
        minBlock = lastBlock;
        console.log ("Continuing querying for All App Drive Transactions from block %s", lastBlock)
    }
    ...
    while (hasNextPage) {
        ...
        try {
            const transactions = await queryGateway(async (url: string) => {
                ...
                const { data } = response.data;
                if (data === undefined) {
                    console.log ("Get All App Transactions DESC... Undefined data returned from Gateway")
                    console.log (response.statusText);
                    return 0;
                } else {
                    const { transactions } = data;
            ...
            if (transactions === 0) {
                console.log ("%s Gateway returned an empty JSON at %s.  Trying again", timeStamp, lastBlock)
                } else {
                    hasNextPage = transactions.pageInfo.hasNextPage
                    const { edges } = transactions;
                    edges.forEach((edge: any) => {
...
export async function getAllDrives_ASC(start: Date, end: Date, lastBlock: number): Promise<ArFSDriveTx[]> {
    let firstPage : number = 100; // Max size of query for GQL
    let cursor : string = "";
    let driveTxs: ArFSDriveTx[] = [];
    let hasNextPage = true;
