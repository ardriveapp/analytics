...
export async function main () {
    ...
    await asyncForEach(status.distributions, async (distribution: { run: number; startTime: Date, time: number; expend: number; totalRecipients: number, totalUploaded: string, transactions: AstatineTxOutput[] }) => {
        console.log ("Start time for this run %s", distribution.startTime);
        await asyncForEach(distribution.transactions, async(transaction: AstatineTxOutput) => {
